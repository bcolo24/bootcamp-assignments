...
router.get('/dish/:num', (req, res) => {
  if(/^[0-9]+$/.test(req.params.num)){
    return res.render('dish', dishes[parseInt(req.params.num) - 1]);
  }
  // use an error handlebars file instead...
  // return res.render('error');
  return res.status(500).json({message: "Thats not a number"});
});
